...
Redaction MUST be applied before final contract-bound checks. If redaction expansion would exceed a diagnostic field's bound, the diagnostic MUST be rejected fail-closed; if it would exceed an optional producer-outcome field's bound, that field MUST be omitted rather than truncated or exported out of schema. Redaction MUST be idempotent: applying it to already sanitized text MUST not add, remove, or duplicate structural delimiters.
