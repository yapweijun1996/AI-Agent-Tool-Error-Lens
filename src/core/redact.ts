...
const AUTHORIZATION_PATTERN = /(?<![A-Za-z0-9])(authorization)(?![A-Za-z0-9])(\s*[:=]\s*)(?:(['"])([A-Za-z][A-Za-z0-9_-]*)\s+[^'"\r\n]*\3|([A-Za-z][A-Za-z0-9_-]*)\s+[^\s,;]+)/giu;
...
export function redactText(value: string): string {
  ...
  sanitized = sanitized.replace(AUTHORIZATION_PATTERN, (_match, key, separator, quote, quotedScheme, unquotedScheme) => {
    const scheme = quotedScheme ?? unquotedScheme;
    return `${key}${separator}${quote ?? ""}${scheme} [REDACTED]${quote ?? ""}`;
  });
