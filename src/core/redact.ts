const KEY_VALUE_PATTERN = /(?<![A-Za-z0-9])(['"]?)(api[-_]?key|apikey|access[-_]?key|access[-_]?token|refresh[-_]?token|client[-_]?secret|private[-_]?key|secret[-_]?access[-_]?key|secret[-_]?key|session[-_]?token|id[-_]?token|oauth[-_]?token|token|password|passwd|secret|database[-_]?url|db[-_]?url|signed[-_]?url)(?![A-Za-z0-9])\1(\s*[:=]\s*)(?:"[^"\r\n]*"|'[^'\r\n]*'|[^\s,;]+)/giu;
