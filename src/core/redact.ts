const KEY_VALUE_PATTERN = /\b(api[-_]?key|apikey|access[-_]?token|refresh[-_]?token|client[-_]?secret|private[-_]?key|secret[-_]?key|session[-_]?token|id[-_]?token|oauth[-_]?token|token|password|passwd|secret|database[-_]?url|db[-_]?url|signed[-_]?url)\b(\s*[:=]\s*)(?:"[^"\r\n]*"|'[^'\r\n]*'|[^\s,;]+)/giu;
const AUTHORIZATION_PATTERN = /\b(authorization)\b(\s*[:=]\s*)([A-Za-z][A-Za-z0-9_-]*\s+)[^\s,;]+/giu;
