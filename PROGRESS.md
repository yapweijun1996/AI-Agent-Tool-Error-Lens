...
| Runtime dependency audit | Pass | `npm ls --omit=dev --depth=0` reports no runtime dependencies and `npm audit --omit=dev --audit-level=high` reports 0 vulnerabilities; the full suite also passes in network-isolated Node 22/24 Linux containers using the workspace dependency tree |
