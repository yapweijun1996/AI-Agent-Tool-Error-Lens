...
| Runtime dependency audit | Pass | `npm ls --omit=dev --depth=0` reports no runtime dependencies and `npm audit --omit=dev --audit-level=high` reports 0 vulnerabilities; archived-workspace offline `npm ci` and the full suite also pass in network-isolated Node 20.11.0/22/24 Linux containers |
